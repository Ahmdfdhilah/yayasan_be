...
class BoardGroupRepository:
    ...
    async def update(self, board_group_id: int, update_data: dict) -> BoardGroup:
        ...
        if new_display_order:
            ...
            current_display_order = current_group.display_order if current_group else None
            
            if current_display_order != new_display_order:
                # Shift existing groups
                await self._shift_display_orders(new_display_order, shift_up=True, exclude_id=board_group_id)
        
        stmt = update(BoardGroup).where(BoardGroup.id == board_group_id).values(**update_data).returning(BoardGroup)
    ...
    async def get_by_id_with_members(self, board_group_id: int) -> Optional[BoardGroup]:
        ...
        query = select(BoardGroup).options(selectinload(BoardGroup.members)).where(BoardGroup.id == board_group_id)
        result = await self.session.execute(query)
        return result.scalar_one_or_none()

    async def get_all_ordered(self) -> List[BoardGroup]:
        """Get all board groups ordered by display_order."""
        query = select(BoardGroup).options(selectinload(BoardGroup.members)).order_by(BoardGroup.display_order.asc())
        result = await self.session.execute(query)
    ...
    async def _shift_display_orders(self, from_order: int, shift_up: bool = True, exclude_id: Optional[int] = None) -> None:
        ...
        if shift_up:
            stmt = update(BoardGroup).where(*conditions).values(display_order=BoardGroup.display_order + 1)
        else:
            stmt = update(BoardGroup).where(*conditions).values(display_order=BoardGroup.display_order - 1)
...
class BoardMemberRepository:
    ...
    async def update(self, board_member_id: int, update_data: dict) -> BoardMember:
        ...
        if new_group_id and new_member_order:
            ...
            current_member_order = current_member.member_order if current_member else None
            
            # If group or order changed, handle shifting
            if (current_group_id != new_group_id) or (current_member_order != new_member_order):
                await self._shift_member_orders(new_group_id, new_member_order, shift_up=True, exclude_id=board_member_id)
        
        stmt = update(BoardMember).where(BoardMember.id == board_member_id).values(**update_data).returning(BoardMember)
        result = await self.session.execute(stmt)
        await self.session.commit()
        return result.scalar_one()
    ...
    async def get_by_group_id(self, group_id: int) -> List[BoardMember]:
        ...
        query = select(BoardMember).where(BoardMember.group_id == group_id).order_by(BoardMember.member_order.asc())
        result = await self.session.execute(query)
        return list(result.scalars().all())

    async def _shift_member_orders(self, group_id: int, from_order: int, shift_up: bool = True, exclude_id: Optional[int] = None) -> None:
        ...
        if shift_up:
            stmt = update(BoardMember).where(*conditions).values(member_order=BoardMember.member_order + 1)
        else:
            stmt = update(BoardMember).where(*conditions).values(member_order=BoardMember.member_order - 1)
